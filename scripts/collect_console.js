...
(async () => {
  ...
  try {
    const res = await page.goto('http://localhost:5173', { waitUntil: 'networkidle', timeout: 15000 });
    logs.push({ type: 'navigation', status: res ? res.status() : 'no-response', url: res ? res.url() : 'unknown' });
  } catch (e) {
