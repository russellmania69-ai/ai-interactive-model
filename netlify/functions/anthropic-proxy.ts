...
const handler: Handler = async (event: HandlerEvent, _context: HandlerContext) => {
  const headers = defaultNetlifyHeaders();
  ...
  if (!apiKey) return { statusCode: 501, headers, body: JSON.stringify({ error: 'Anthropic API key not configured (ANTHROPIC_API_KEY)' }) };
  ...
  if (!input) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing `input` in request body' }) };
  ...
  try {
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Proxy request failed', details: String(err?.message || err) }) };
  }
};
